...
class Worker:
    ...
    def __init__(self, q, date_last, date_to):
        ...
        self.proxy = q.get()
        logger.info(f'Прокси: {self.proxy}')

    def api_req(self, q, page, date_from, date_to, retry=2):
        params = {
            'per_page': 100,
            'page': page,
        ...
        try:
            ...
            q.put(self.proxy)
            self.proxy = q.get()
            return self.api_req(q, page, date_from, date_to)
            ...
            if retry:
                ...
                return self.api_req(q, page, date_from, date_to, retry=(retry - 1))
            else:
                q.put(self.proxy)
                self.proxy = q.get()
                return self.api_req(q, page, date_from, date_to)
            ...
            if retry:
                if data['items'] == []:
                    data = self.api_req(q, page, date_from, date_to, retry=(retry - 1))
            else:
                return None
            return data
        finally:
    ...
    def get_time_step(self, date_left, date_right, q):
        if date_left < 0:
            date_left = 0
        data = self.api_req(q, 0, self.convert_seconds_in_date(date_left), self.convert_seconds_in_date(date_right))
        if data == None:
            return date_left
    ...
    def make_req_ids(self, q, id, retry=2):
        url = f'{URL}/{id}'
        req = None
        try:
            ...
            if retry:
                time.sleep(10)
                logger.info(f'{err}. retry {retry} proxy {self.proxy}')
                return self.make_req_ids(q, id, retry=(retry - 1))
            else:
                q.put(self.proxy)
                self.proxy = q.get()
                return self.make_req_ids(q, id)
    ...
    def process_data_from_queue(self):
        try:
            conn = psycopg2.connect(database='headhunter_roles', user='postgres', host='localhost', port='5432',
                                    password='2280')
            cur = conn.cursor()
    ...
    def run(self, q):
        ...
        while self.date_to != 0:
            next_date = self.get_time_step(self.date_to - DEFAULT_MAX_STEP_SIZE, self.date_to, q)
            print(self.date_to)
        ...
        while not self.queue_a.empty():
            ...
            for page in range(20):
                data = self.api_req(q, page, date_step[0], date_step[1])
                if data == None:
                    break
